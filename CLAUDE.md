...
- 🟠 **cn() doit passer par extendTailwindMerge avec les tailles maison (text-page, text-section, text-produit, text-courant, text-legende) déclarées en font-size : sinon tailwind-merge prend text-legende pour une COULEUR et supprime text-primary-foreground. Toute nouvelle taille dans tailwind.config.ts s'ajoute aussi dans src/lib/utils.ts** *(05/09/2026)*
  *05/09/2026 : tous les boutons compacts (Créer un compte, Se connecter, Choisir le lieu) affichaient du texte béton #29313d sur latérite, contraste 2,56:1 au lieu de 5,11:1 ; la classe text-primary-foreground manquait dans le DOM alors que button.tsx la posait — twMerge l'avait retirée à cause de text-legende*
