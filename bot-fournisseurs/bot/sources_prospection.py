...
REQUETES_DEFAUT = [
    "matériaux de construction Madagascar",
    "parpaing Antananarivo",
    "biriky amidy",
    "sable gravillon Antananarivo",
    "fasika vato Madagascar",
    "ciment Madagascar",
    "tôle Antananarivo",
    "fer à béton Madagascar",
    "briqueterie Madagascar",
    "carrière sable Antananarivo",
    "bois de construction Madagascar",
    "hazo fanamboarana trano",
]
...
# On réutilise le vocabulaire du collecteur : le même mot doit vouloir dire la
# même chose quand on trie une offre et quand on choisit une source.
def _mots_du_metier() -> tuple[str, ...]:
    from .collecteur import MOTS_CONTEXTE, MOTS_MATERIAUX

    return tuple(sorted(set(MOTS_MATERIAUX) | set(MOTS_CONTEXTE) | {
        "materiaux", "construction", "batiment", "btp", "quincaillerie",
        "fanamboarana", "manorina", "batir",
    }))


MOTS_METIER = _mots_du_metier()

# Un nom qui contient ça ne vend pas de matériaux.
MOTS_REPOUSSOIRS = (
    # « tolotr'asa » = offre d'emploi. Un groupe d'emploi du bâtiment noierait
    # la collecte sous des candidatures.
    "emploi", "job", "recrutement", "tolotr'asa", "tolotrasa", "tolotr asa",
    "mitady asa", "offre d emploi", "stage", "rencontre", "amour", "priere",
    "eglise", "musique", "football", "vente de voiture", "moto", "phone",
    "telephone", "vetement", "friperie", "sante", "covoiturage", "taxi",
    "voyage", "billet", "jeux", "pari", "crypto",
    # Le voisinage immobilier est le piège n°1 d'Akora : « tany » et « vato »
    # sont de vrais matériaux, mais dans « TANY NA TRANO AMIDY NA AHOFA » ils
    # désignent un terrain à vendre. « ahofa » (à louer) ne se dit jamais d'un
    # sac de ciment : c'est le mot qui sépare proprement les deux métiers.
    "ahofa", "immobilier", "location maison", "appartement", "villa a",
    "trano amidy", "tany amidy", "terrain a vendre", "agence immobiliere",
)
...
CATEGORIES_BONNES = (
    "materiaux de construction", "entreprise de construction", "quincaillerie",
    "fournisseur", "grossiste", "entreprise locale", "service commercial",
    "entrepreneur", "carriere", "industrie", "batiment",
)
CATEGORIES_MAUVAISES = (
    "mannequinat", "coach", "gouvernement", "photographe", "artiste",
    "musicien", "restaurant", "eglise", "ecole", "voyage", "hotel",
    "association caritative", "media", "immobilier", "agent immobilier",
)
...
def noter(candidat: dict, requetes: list[str], mots_metier=MOTS_METIER,
    ...
    # Lieu (10) — Madagascar, et si possible la zone travaillée.
    lieu = _sans_accents(
        (candidat.get("lieu") or "") + " " + (candidat.get("nom") or "")
    )
    if re.search(r"antananarivo|tananarive|(?<![a-z])tana(?![a-z])", lieu):
        p, motif = 10, "Antananarivo"
    elif "madagascar" in lieu or "mada" in lieu:
        p, motif = 7, "Madagascar"
    else:
