...
def test_les_polices_fantaisie_de_facebook():
    ...
    """
    assert sp._sans_accents("𝘾𝙄𝙈𝙀𝙉𝙏 𝙈𝘼𝘿𝘼") == "ciment mada"
    note = sp.noter(_c(nom="𝘾𝙄𝙈𝙀𝙉𝙏 𝙈𝘼𝘿𝘼", effectif=40_000, genre="page",
                       categorie="Materiaux de construction"), [])
    assert note["note"] >= 75, note


# -- Ce que la note doit trancher ------------------------------------------
def test_un_gros_groupe_tres_actif_passe_devant():
    fort = sp.noter(_c(nom="Materiaux de construction BTP", effectif=68_000,
                       rythme_par_jour=90, lieu="Madagascar"), [])
    faible = sp.noter(_c(nom="Materiaux de construction BTP", effectif=300,
...
def test_la_taille_departage_a_rythme_egal():
    ...
    """
    gros = sp.noter(_c(nom="materiaux construction", effectif=122_000, rythme_par_jour=90), [])
    moyen = sp.noter(_c(nom="materiaux construction", effectif=6_000, rythme_par_jour=90), [])
    assert gros["note_brute"] > moyen["note_brute"]


def test_la_categorie_facebook_prime_sur_le_nom():
    """Un nom qui parle de béton, une catégorie qui dit « Coach »."""
    note = sp.noter(_c(nom="Beton et bien-etre", effectif=3_000, genre="page",
                       categorie="Coach personnel"), [])
    assert note["note"] < 40, note
...
def test_un_groupe_prive_est_signale_et_penalise():
    ouvert = sp.noter(_c(nom="depot materiaux tana", effectif=20_000, rythme_par_jour=60), [])
    ferme = sp.noter(_c(nom="depot materiaux tana", effectif=20_000, rythme_par_jour=60,
                        prive=True), [])
    assert ferme["note"] < ouvert["note"]
    assert any("rejoindre" in a for a in ferme["alertes"])


def test_un_groupe_immobilier_ne_passe_pas_pour_un_depot():
    """« TANY NA TRANO AMIDY NA AHOFA » : « tany » et « vato » sont de vrais
    matériaux, mais ici ils désignent un terrain à vendre. C'était le premier
    de la prospection Akora, à 100/100."""
    note = sp.noter(_c(nom="Tany/Trano Amidy sy ahofa eto Antananarivo",
                       effectif=146_000, rythme_par_jour=90), [])
    assert note["note"] <= 62, note
    assert any("ahofa" in a for a in note["alertes"]), note["alertes"]
