...
/**
 * Déclaration d'accessibilité — construite par l'audit du 05/09/2026 (A-06).
 *
 * Madagascar n'impose pas (encore) de déclaration ; on la publie parce que
 * l'audience cible (téléphone d'entrée de gamme, 3G, parfois lecteur d'écran
 * TalkBack) mérite de savoir ce qui marche et ce qui ne marche pas. Le texte
 * dit ce qui a été MESURÉ, avec la date et l'outil, jamais « conforme » sans
 * mesure. À mettre à jour à chaque correction listée ci-dessous.
 */
  ...
  return (
    ...
    >
      ...
      <p>
        ...
        largeur 390 px et 1 280 px), plus le banc automatique du dépôt (`npm run test:a11y`). Résultat :
        <strong> partiellement conforme</strong>.
      </p>
      <p>Ce qui fonctionne :</p>
      <ul>
        <li>Un lien « Aller au contenu » en tête de chaque page ; un seul titre principal par page.</li>
        ...
        <li>Cibles tactiles de 44 px minimum sur les boutons ; zoom jamais bloqué.</li>
      </ul>

      <h2>Défauts connus et calendrier</h2>
      <ul>
        <li>
          <strong>Contraste des boutons compacts</strong> (« Créer un compte », « Se connecter » dans
          l'en-tête, « Choisir le lieu ») : texte foncé sur latérite, 2,6:1 au lieu de 4,5:1 — corrigé
          dans la version du 6 septembre 2026.
        </li>
        <li>
          <strong>Curseur de quantité</strong> (comparateur, calculateurs) : le curseur n'avait pas de
          nom lu par le lecteur d'écran — corrigé dans la même version.
        </li>
        <li>
          <strong>Tableau de comparaison</strong> : sur petit écran il défile horizontalement ; un
          libellé masqué le faisait déborder de la page — corrigé.
        </li>
        <li>
          <strong>Petites cibles</strong> : les puces de produits sur la fiche d'un fournisseur et les
          cases à cocher de l'inscription (20 px) restent sous 24 px — correction prévue avant fin
          septembre 2026.
        </li>
        <li>
        ...
        <li>
          Les noms de dépôts en malgache (« Fivarotan-kazo… ») ne sont pas marqués comme changement de
          langue : la synthèse vocale les prononce à la française.
        </li>
      </ul>
