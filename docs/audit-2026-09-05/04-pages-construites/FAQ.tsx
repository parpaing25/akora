...
/**
 * FAQ — construite par l'audit du 05/09/2026 (C-04). Chaque réponse reprend
 * un fait déjà écrit ailleurs sur le site (conditions, vérification, devenir
 * fournisseur, paiement) : une FAQ qui invente une règle crée un litige.
 *
 * Le JSON-LD FAQPage est généré depuis la même liste : une seule source, pas
 * deux textes qui divergent. Les questions sont formulées comme les gens les
 * posent (moteurs de réponse IA, recherche vocale).
 *
 * `[À COMPLÉTER]` : délai de confirmation des paiements (Q2 de la fiche
 * d'identité) et numéro de téléphone (Q5). Ne pas publier avec ces crochets.
 */
const QUESTIONS: { id: string; question: string; reponse: string; lien?: { vers: string; texte: string } }[] = [
  ...
  },
  {
    ...
    reponse:
      "Par e-mail à contact@akora.fonenako.mg, ou par téléphone et WhatsApp au [À COMPLÉTER : +261 …] du lundi au samedi, 8 h – 17 h. Pour une commande en cours, le plus rapide reste le fournisseur, joignable depuis la page de la commande.",
    lien: { vers: "/contact", texte: "Page contact" },
  },
];

/** FAQPage Schema.org, texte brut (les moteurs n'acceptent pas de HTML dans `text`). */
const DONNEES_FAQ: Record<string, unknown> = {
...
export default function FAQ() {
  return (
    ...
    >
      {/* Sommaire : 13 questions, l'utilisateur mobile saute directement à la sienne. */}
      <nav aria-label="Questions" className="not-prose mb-6 rounded-md bg-muted p-3">
        <ol className="grid gap-1 text-legende sm:grid-cols-2">
          {QUESTIONS.map((q) => (
            <li key={q.id}>
              <a href={`#${q.id}`} className="lien-souligne">
                {q.question}
              </a>
            </li>
          ))}
