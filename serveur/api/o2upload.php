...
// Secrets serveur (O2SWITCH_UPLOAD_API_KEY...) depuis ~/.env_akora
...
function ak_generate_thumb($src, $dst, $maxDim = 480, $quality = 72) {
...
if (!$authenticated) {
  ...
  if (preg_match('/Bearer\s+(.+)$/i', $authHeader, $matches)) {
    ...
    $supabaseUrl = getenv('SUPABASE_URL') ?: '' /* renseigne dans .env_akora */;
    $anonKey = getenv('SUPABASE_ANON_KEY') ?: '' /* renseigne dans .env_akora */;
...
if (preg_match('/\.(jpe?g|png|webp)$/i', $filename)) {
  ...
  if (@ak_generate_thumb($targetPath, $thumbPath, 480, 72)) {
    $schemeT = (!empty($_SERVER['HTTPS']) && $_SERVER['HTTPS'] !== 'off') ? 'https' : 'http';
    $hostT = $_SERVER['HTTP_HOST'] ?? 'akora.fonenako.mg';
    $thumbUrl = $schemeT . '://' . $hostT . '/uploads/' . $folder . '/' . $thumbRel;
  }
}

$scheme = (!empty($_SERVER['HTTPS']) && $_SERVER['HTTPS'] !== 'off') ? 'https' : 'http';
$host = $_SERVER['HTTP_HOST'] ?? 'akora.fonenako.mg';
